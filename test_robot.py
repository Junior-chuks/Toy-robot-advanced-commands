...
class MyFunctions(unittest.TestCase):
    def test_history(self):
        ...
        with captured_io(StringIO("forward 10\nback 20\nright\nright\nleft\n")):
            commands = robot.get_command("Junior")
            command_1 = robot.get_command("Junior")
            command_2 = robot.get_command("Junior")
            command_3 = robot.get_command("Junior")
            command_4 = robot.get_command("Junior")
            self.assertEqual(robot.command_history(commands,["5","2"]),["forward 10"])
            self.assertEqual(robot.command_history(command_1,["5","2"]),["forward 10","back 20"])
            self.assertEqual(robot.command_history(command_2,["5","2"]),["forward 10","back 20","right"])
            self.assertEqual(robot.command_history(command_3,["5","2"]),["forward 10","back 20","right","right"])
            self.assertEqual(robot.command_history(command_4,["5","2"]),["forward 10","back 20","right","right","left"])
